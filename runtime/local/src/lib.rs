...
use frame_support::{
    ...
    traits::{Currency, FindAuthor, Imbalance, KeyOwnerProofSystem, Nothing, OnUnbalanced, SortedMembers, Time},
    weights::{
...
pub use rio_primitives::{
    AccountIndex, Amount, Balance, CurrencyId, DataProviderId,
    Moment, Price,
};
...
pub mod impls;
use impls::SlowAdjustingFeeUpdate;

use frame_system::{EnsureOneOf, EnsureRoot, EnsureSignedBy, RawOrigin, EnsureSigned};
...
use rio_protocol::{RFUEL,STAKING_POOL_MARKER};
...
impl rio_staking_pool::Config for Runtime {
    type DefaultPrice = DefaultPrice;
    type Event = Event;
    type MarkerCurrency = rio_assets::frame_currency::Compat<Runtime,StakingPoolMarker>;
    type MaximumPerBlockReward = MaximumPerBlockReward;
    type MinimumStakeBalance = MinimumStakeBalance;
    type StakeCurrency = rio_assets::frame_currency::Compat<Runtime,Rfuel>;
...
impl rio_gateway::Config for Runtime {
    type Event = Event;
    type Currency = Currencies;
    type WeightInfo = ();
}

type EnsureRootOrManager = EnsureOneOf<AccountId, EnsureRoot<AccountId>, EnsureSignedBy<RioRoot, AccountId>>;

impl rio_root::Config for Runtime {
    type Event = Event;
...
impl rio_payment::Config for Runtime {
    type Currency = Balances;
...
impl rio_assets_ext::Config for Runtime {
    type Event = Event;
}
...
impl rio_prices::Config for Runtime {
    type Event = Event;
    type Source = Oracle;
...
pub type MomentOf<T, I = ()> = <<T as orml_oracle::Config<I>>::Time as Time>::Moment;
pub type TimestampedValueOf<T, I = ()> = orml_oracle::TimestampedValue<<T as orml_oracle::Config<I>>::OracleValue, MomentOf<T, I>>;
pub type TimeStampedPrice = TimestampedValueOf<Runtime>;
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic
    {
        ...
        RioAssets: rio_assets::{Pallet, Call, Storage, Config<T>, Event<T>},
        RioAssetsExt: rio_assets_ext::{Pallet, Call, Storage, Event<T>},
        RioGateway: rio_gateway::{Pallet, Call, Storage, Config<T>, Event<T>},
        RioRoot: rio_root::{Pallet, Call, Storage, Config<T>, Event<T>},
        RioPayment: rio_payment::{Pallet, Call, Storage},
        RioPaymentFee: rio_payment_fee::{Pallet, Call, Storage, Config<T>, Event<T>},
        RioPrices: rio_prices::{Pallet, Call, Storage, Event},
        RioStakingPool: rio_staking_pool::{Pallet, Call, Storage, Event<T>},
    }
);
...
impl_runtime_apis! {
    ...
    impl rio_gateway_rpc_runtime_api::OracleApi<
        Block,
        DataProviderId,
        CurrencyId,
        TimeStampedPrice,
