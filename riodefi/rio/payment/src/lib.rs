...
use frame_support::{
    ...
    traits::{Currency, ExistenceRequirement, Get, Imbalance, OnUnbalanced, WithdrawReasons, SameOrOther},
    weights::{
...
use sp_runtime::{
    traits::{
        Bounded, Convert, DispatchInfoOf, Dispatchable, PostDispatchInfoOf, SaturatedConversion,
    ...
    DispatchError, FixedPointNumber, FixedPointOperand, FixedU128, Perquintill, RuntimeDebug, Perbill
};
use sp_std::prelude::*;
...
//pub use rio_primitives::types::TargetedFeeAdjustment;
        ...
        M: Get<Multiplier>,
{
    fn convert(previous: Multiplier) -> Multiplier {
        ...
        let normal_max_weight = T::AvailableBlockRatio::get()
        ...
        fn integrity_test() {
            ...
            assert!(
                ...
                Multiplier::checked_from_integer(
                    <T as frame_system::Config>::BlockWeights::get().max_block.try_into().unwrap()
                ).unwrap(),
            );
            ...
            let mut target =
                ...
                (T::AvailableBlockRatio::get() * T::BlockWeights::get().max_block);
            ...
            sp_io::TestExternalities::new_empty().execute_with(|| {
                ...
                //TODO should we somehow use set block number here due to set block limit not present any more
        ...
        BalanceOf<T>: FixedPointOperand,
{
    ...
    fn compute_fee_raw(
        ...
        if pays_fee == Pays::Yes {
            ...
            //     .saturating_add(tip);
            // debug!("[compute_fee]|final_fee:{:?}|base_fee:{:?}|adjusted_weight_fee:{:?}|tip:{:?}|fixed_len_fee:{:?}|unadjusted_weight_fee:{:?}",
            //             final_fee, base_fee, adjusted_weight_fee, tip, fixed_len_fee, unadjusted_weight_fee);
            // final_fee
            // tmp fix the fee to 0.1 Rfuel ()
            let static_fee: BalanceOf<T> = (1_000_000_000_000 / 10).saturated_into();
            static_fee + tip
        } else {
            tip
        }
    ...
    fn weight_to_fee(weight: Weight) -> BalanceOf<T> {
        ...
        let capped_weight = weight.min(<T as frame_system::Config>::BlockWeights::get().max_block);
...
pub struct ChargeTransactionPayment<T: Config + Send + Sync> {
    #[codec(compact)]
    fee: BalanceOf<T>,
        ...
        BalanceOf<T>: Send + Sync + FixedPointOperand,
{
    ...
    pub fn from(fee: BalanceOf<T>, _fee_exchange: Option<FeeExchange<BalanceOf<T>>>) -> Self {
        Self {
            fee, /*fee_exchange*/
        }
    }
    ...
    fn withdraw_fee(
        ...
        (BalanceOf<T>, BalanceOf<T>, Option<NegativeImbalanceOf<T>>),
    ...
    > {
        ...
        let bought_fee = T::BuyFeeAsset::buy_fee_asset(who, fee, &exchange).map_err(|e| {
            let code = match e {
                DispatchError::Module { message, .. } => message
                    .map(error::error_code::buy_fee_asset_error_msg_to_code)
                    .unwrap_or(error::error_code::UNKNOWN_BUY_FEE_ASSET),
                _ => error::error_code::UNKNOWN_BUY_FEE_ASSET,
            };

            TransactionValidityError::Invalid(InvalidTransaction::Custom(code))
        })?;
        if !bought_fee.is_zero() {
            let _ = T::Currency::deposit_creating(who, bought_fee);
        }
        // }
        ...
        match T::Currency::withdraw(
            ...
            Ok(imbalance) => Ok((fee, bought_fee, Some(imbalance))),
...
impl<T: Config + Send + Sync> sp_std::fmt::Debug for ChargeTransactionPayment<T> {
    ...
    fn fmt(&self, f: &mut sp_std::fmt::Formatter) -> sp_std::fmt::Result {
        write!(f, "ChargeTransactionPayment<{:?}>", self.fee)
    }
    #[cfg(not(feature = "std"))]
        ...
        T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    fn validate(
        ...
        let (fee, _, _) = self.withdraw_fee(who, info, len)?;
        Ok(ValidTransaction {
    ...
    fn pre_dispatch(
        ...
        let (fee, bought_fee, imbalance) = self.withdraw_fee(who, info, len)?;
        Ok((self.fee, who.clone(), imbalance, fee, bought_fee))
    }
    ...
    fn post_dispatch(
        ...
        let (tip, who, imbalance, fee, bought_fee) = pre;
        if let Some(payed) = imbalance {
            let mut actual_fee = Module::<T>::compute_actual_fee(len as u32, info, post_info, tip);

            if actual_fee < bought_fee {
                // real cast less then bought fee, need to return some fee to bought_fee
                let diff = bought_fee - actual_fee;
                T::BuyFeeAsset::refund_fee_asset(&who, diff).expect("refund should not fail");
                // actual_fee < bought_fee, thus NativeCurrency would refund all. bought fee is
                // refunded above, thus fee-bought_fee is all native currency balance
                actual_fee = bought_fee;
            } else {
                // actual_fee >= bought_fee, thus bought_fee would not refund, thus actual_fee
                // should be all deducted from fee
            }
...
mod tests {
    ...
    parameter_types! {
        ...
        pub const RioBlockWeights: limits::BlockWeights = limits::BlockWeights::builder()
		.base_block(10)
		...
		.build_or_panic();
        pub RioBlockLength: limits::BlockLength =
    ...
    impl frame_system::Config for Runtime {
        ...
        // BlockWeights.WeightsPerClass.base_extrinsic
        type ExtrinsicBaseWeight = ExtrinsicBaseWeight;
        // BlockWeights.WeightsPerClass.max_extrinsic
    ...
    impl rio_assets::Config for Runtime {
        type Event = Event;
        type Balance = u64;
        type Amount = i64;
        type CurrencyId = u32;
    ...
    impl rio_payment_fee::Config for Runtime {
        type Event = Event;
    ...
    type RioAssets = rio_assets::Module<Runtime>;
    type RioPaymentFee = rio_payment_fee::Module<Runtime>;
    ...
    impl ExtBuilder {
        ...
        pub fn build(self) -> sp_io::TestExternalities {
            ...
            // rio genesis
            let init_asset = vec![(
                rio_protocol::LOCKED_RFUEL,
                rio_assets::AssetInfo {
                    symbol: b"LOCKED_RFUEL".to_vec(),
                    name: b"Locked RioFuel".to_vec(),
                    decimals: 12,
                    desc: b"RioChain Locked RioFuel Token".to_vec(),
                    chain: rio_assets::Chain::Rio,
                },
                rio_assets::Restriction::Transferable.into(),
                vec![],
            )];
            rio_assets::GenesisConfig::<Runtime> { init: init_asset }
                .assimilate_storage(&mut t)
                .unwrap();
            t.into()
        }
    ...
    fn signed_ext_length_fee_is_also_updated_per_congestion() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                assert!(
                    ChargeTransactionPayment::<Runtime>::from(10, None) // tipped
            ...
            .execute_with(||
                {
                    ...
                    assert_eq!(
                        ...
                        RuntimeDispatchInfo {
                            ...
                            5 * 2 /* base * weight_fee */
                                + len as u64  /* len * 1 */
                                + info.weight.min(RioBlockWeights::get().max_block) as u64 * 2 * 3 / 2 /* weight */
                        },
                    );
    ...
    #[test]
    fn signed_extension_transaction_payment_work_with_buy_fee() {
        ExtBuilder::default()
            .balance_factor(10)
            .base_weight(5)
            .build()
            .execute_with(|| {
                RioAssets::deposit(rio_protocol::LOCKED_RFUEL, &1, 30).unwrap();
                RioAssets::reserve(rio_protocol::LOCKED_RFUEL, &1, 30).unwrap();
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &1),
                    30
                );
                ...
                let pre = ChargeTransactionPayment::<Runtime>::from(5 /* tipped */, None)
                    .pre_dispatch(&1, CALL, &info_from_weight(5), len)
                    .unwrap();
                assert_eq!(Balances::free_balance(1), 100 - 10 - 5);
    ...
    #[test]
    fn signed_extension_transaction_payment_multiplied_refund_works_with_by_fee() {
        ExtBuilder::default()
            .balance_factor(10)
            .base_weight(5)
            .build()
            .execute_with(|| {
                RioAssets::deposit(rio_protocol::LOCKED_RFUEL, &2, 200).unwrap();
                RioAssets::reserve(rio_protocol::LOCKED_RFUEL, &2, 200).unwrap();
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &2),
                    200
                );
                ...
                let pre = ChargeTransactionPayment::<Runtime>::from(5 /* tipped */, None)
                    ...
                    .unwrap();
                // 5 base fee, 10 byte fee, 3/2 * 100 weight fee, 5 tip
                assert_eq!(Balances::free_balance(2), 200 - (5 + 10 + 150 + 5 - 105));
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &2),
                    0
                );
                ...
                .is_ok());
                // 75 (3/2 of the returned 50 units of weight) is refunded
                // 200 - (5 + 10 + 150 + 5 - 105) + 75 - 200 > 0, thus use 200
                assert_eq!(Balances::free_balance(2), 200);
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &2),
                    200 - (5 + 10 + 150 + 5 - 105) + 75 - 200 // 10
                );
                ...
                let pre = ChargeTransactionPayment::<Runtime>::from(5 /* tipped */, None)
                    ...
                    .unwrap();
                // 5 base fee, 10 byte fee, 3/2 * 100 weight fee, 5 tip
                assert_eq!(Balances::free_balance(2), 200 - (5 + 10 + 150 + 5 - 10));
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &2),
                    0
                );
                ...
                .is_ok());
                // 75 (3/2 of the returned 50 units of weight) is refunded
                // 200 - (5 + 10 + 150 + 5 - 10) + 75 < 200, thus
                assert_eq!(
                    Balances::free_balance(2),
                    200 - (5 + 10 + 150 + 5 - 10) + 75
                );
                assert_eq!(
                    RioAssets::reserved_balance(rio_protocol::LOCKED_RFUEL, &2),
                    0,
